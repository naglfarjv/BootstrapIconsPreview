...
function cargaEjemplos() {
    var clase = $("#ddlIcono").val();
    var svg = '<i class="ejemplo ' + clase + '"></i>';

    $('.ejemplo').replaceWith(svg);
    $('#spanClase').html('"' + clase + '"');
...
function cambiaColor() {
    ...
    $('.ejemplo').css('color', color);
}

function asignaDdl(valor) {
    $('#ddlIcono').val(valor).trigger('change');
}
...
function creaTarjetas() {
    ...
    arrCod.forEach(function(arrCod, index) {
        html = '';
        html =
            '<li>' +
            '<div class="p-3 py-4 mb-2 bg-light text-center rounded" onclick="asignaDdl(\'' + arrCod + '\');">' +
            '<i class="' + arrCod + '"></i>' +
            '<div class="name text-muted text-decoration-none text-center pt-1">' + arrCod + '</div>' +
            '</div>' +
            '</li>';

        $("#listaTarjetas").append(html);

        html2 = '';
        html2 = '<option value="' + arrCod + '">' + arrCod.substring(6, arrCod.length) + '</option>';
        $("#ddlIcono").append(html2);
    });
}